...
from .models import LoginActivity, Registration, VehiclePass


# 1. Log every user login
@receiver(user_logged_in)
def log_user_login(sender, request, user, **kwargs):
    LoginActivity.objects.create(user=user)


# 2. Auto-update registration when submitted
@receiver(post_save, sender=Registration)
def handle_registration_submission(sender, instance, created, **kwargs):
    """
    When a user submits an application, it enters 'application submitted'.
    After 5 working days (if untouched), it can be escalated automatically.
    For now, we just leave a placeholder for scheduling.
    """
    if created and instance.status == 'application submitted':
        # TODO: Hook Celery/Django-Q to check this after 5 days.
        # Example idea: auto-change to "initial approval" after 5 days.
        pass


# 3. Keep Registration in sync with VehiclePass creation
@receiver(post_save, sender=VehiclePass)
def update_registration_on_pass_issue(sender, instance, created, **kwargs):
    """
    When a VehiclePass is issued, mark the linked Registration as
    'sticker released' and set the release date.
    """
    if created:
        with transaction.atomic():
            registration = Registration.objects.filter(vehicle=instance.vehicle).last()
            if registration:
                registration.status = 'sticker released'
                registration.sticker_released_date = timezone.now().date()
                registration.save(update_fields=['status', 'sticker_released_date'])
